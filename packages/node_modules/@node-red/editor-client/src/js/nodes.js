            ...
            for (var d3 in n._def.defaults) {
                if (n._def.defaults.hasOwnProperty(d3)) {
                    if (n._def.defaults[d3].type) {
                        if (node_map[n[d3]]) { 
                            var node = node_map[n[d3]];
                            n[d3] = node.id;
                            if (node.users) { // No users for subflow instances node
                                if (node.users.indexOf(n) === -1) {
                                    node.users.push(n);
                                }
                            }
                        } else if (subflow_map[n[d3]]) { // Replace subflow template node
                            n[d3] = subflow_map[n[d3]].id;
